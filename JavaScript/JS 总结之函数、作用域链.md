...
每个执行环境都有一个变量对象，存放着环境中定义的所有变量和函数，是作用域链形成的前置条件。但我们无法使用这个变量对象，该对象主要是给 JS 引擎使用的。具体可以查看[《JS 总结之变量对象》]()。这里暂时理解为：
...
对于 fn2 来说，能访问到 **fn2 执行环境**、**fn1 执行环境** 和 **全局执行环境** 的变量对象（所有变量和函数）。

对于 fn1 来说，能访问到 **fn1 执行环境** 和 **全局执行环境** 的变量对象（所有变量和函数）。

总结为一句：函数内部能访问到函数外部的值，函数外部无法范围到函数内部的值。

所以引出了闭包的概念，查看总结：[《JS 总结之闭包》](https://github.com/KaronAmI/blog/issues/26)
